...
import express, { json } from 'express';  //express 서버구축
import { readFile, writeFile } from 'fs'; // 파일 시스템 모듈 일을 읽고 쓰게 해줌

const app = express();  // Express 인스턴스 생성
const PORT = 3001;  //포트번호 30001번으로
app.use(cors());  // 모든 도메인에서의 요청을 허용
app.use(json()); //본문에 직접 파싱가능하게 해줌

// 고객 데이터를 불러오는 API
//GET 요청을 받으면 실행되는 핸들러
app.get('/customers', (req, res) => {
    readFile('../json/user.json', 'utf8', (err, data) => {
      if (err) {
  ...
  readFile('../json/user.json', 'utf8', (err, data) => {
    if (err) {
      ...
      if (customerIndex !== -1) {
        ...
        writeFile('../json/user.json', JSON.stringify(customers, null, 2), (err) => {
          if (err) {
